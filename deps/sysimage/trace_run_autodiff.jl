...
function compile_autodiff()
    ...
    NN1 = Network(NN1.layers[1:1]) # Small network for testing
    ...
    result = VeryDiff.DiffForwardResult(nothing)

    diff_forward_result = VeryDiff.forward_diff(N, diff_zono, P, result, VeryDiff.diff_bound_loss)
    println("diff_forward loss: ", diff_forward_result)
    println("diff_forward result: ", typeof(result.output))
    ...
    gs = @time Enzyme.autodiff(
        ...
        Const(VeryDiff.forward_diff),
        Active,
        Duplicated(N, N_copy),
        Duplicated(diff_zono, Z_copy),
        Const(P),
        Const(result),
        Const(VeryDiff.diff_bound_loss)
    )
