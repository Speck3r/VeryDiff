...
"""
function sigmoid_crit_overapprox(l, u, a, b)
    xs_all = [l, u]
...
"""
function sigmoid_crit_overapprox_cheby(l, u, ps)
    a = convert_cheby_lin_to_monomial(ps, l, u)
    return sigmoid_crit_overapprox(l, u, a[2], a[1])
end


function approx_sigmoid_lin(l::N, u::N; verbosity=0, tol=N(1e-10), max_iter=10, cheby=true) where N<:Number
    if l == u 
    ...
    degree = 1
    p_lin, ϵ = remez(gelu, (p, l, u) -> sigmoid_crit_overapprox_cheby(l, u, p), VeryDiff.poly_norm, l, u, degree, 
...
function get_linear_relaxation(L::ONNXSigmoid, lower, upper)
    res = approx_sigmoid_lin.(lower, upper, max_iter=REMEZ_ITERS[], cheby=true)
    λ = getindex.(res, 1)
    β = getindex.(res, 2)
    γ = getindex.(res, 3)
    return λ, β, γ
