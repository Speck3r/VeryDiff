...
function create_random_dense_network(input_dim::Int, layer_dims::Vector{Int}; relu=false, add_const=false)
    ...
    for new_dim in layer_dims
        ...
        if relu
            ...
            # Quick fix to test different activation functions
            #push!(layers, ONNXRelu([relu_input_id], [relu_output_id], "relu_$layer_count"))
            #push!(layers, ONNXLeakyRelu([relu_input_id], [relu_output_id], "leakyrelu_$layer_count",((20.0 * rand()) - 10.0)))
...
function create_random_layer_mutant(layer :: ONNXLeakyRelu{String, Float64})
    return deepcopy(layer)
end
